...
class Toggl {
    ...
    this.client.getTimeEntries(startDate, endDate,
      ...
      timeEntries = timeEntries.filter(function(timeEntry) {
        return !timeEntry.hasOwnProperty('tags') ||
               timeEntry['tags'].indexOf('processed') === -1;
      });
  ...
  /**
   * Adds 'processed' tag to given time entry
   *
   * @param {Object} timeEntry
   * @param {Callable} callback
   */
  addProcessedTagToTimeEntry(timeEntry, callback) {
    // Add 'processed' tag to all grouped time entries
